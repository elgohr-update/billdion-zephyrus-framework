...
{
    /**
     * This method is called before every route call from inherited controllers
     * and makes sure to check if there is an intrusion detection, or an
     * authorization problem and sets the Content Security Policy headers. Any
     * other security considerations that should be checked BEFORE processing
     * any route should be done here.
     *
     * Parent call should ensure check for basic security measures in any
     * application such as CSRF validation, intrusion detection and authorization
     * access.
     *
    ...
    public function before(): ?Response
    {
         ...
         * IntrusionDetectionException. Exception can be thrown to be caught
         * by children controllers if needed or the error handler. It is
         * recommended to catch them here to ensure a proper uniform handling
         * of the basic security controls.
         */
        try {
            parent::before();
             ...
             * represents the severity of the attempt. IntrusionDetection class
             * is a wrapper of the expose library.
             *
             * Be careful about the action chosen to handle such case as there
             * may have false positive for legitimate clients. That is why there
             * are no default action.
             *
            ...
            /**
             * Defines what to do when the CSRF token mismatch. Meaning there's
             * an attempt to access a route with no token or with an expired
             * already use token. By default, treat this as a forbidden access
             * to the route. This will break the middleware chain and immediately
             * return the 403 HTTP code and thus ensure protection of the route
             * processing.
             */
            // Do something (logs, database report, redirect, ...)
            return $this->abortForbidden();
            ...
            /**
             * Defines what to do when the route doesn't meet the authorization
             * requirements. By default, treat this as a forbidden access to the
             * route. This will break the middleware chain and immediately
             * return the 403 HTTP code and thus ensure protection of the route
             * processing.
             */
            // Do something (logs, database report, redirect, ...)
    ...
    /**
     * Defines the application authorizations for all inherited controllers. For
     * a cleaner and maintainable codebase for large projects with a huge
     * quantities or routes with more or less complex authorization, it should
     * be split across multiple middlewares (overriding the before() method) for
     * specific controller to set the related authorizations.
     */
    private function setupAuthorizations()
    {
         ...
         *
         * Blacklist should be consider for a little application where there's
         * only an administrative zone that should be protected.
         *
         * Whitelist should be consider for an application that requires a login
         * and that roles change how the application is accessed.
         */
         ...
         * automatically denied), it may be useful to specify a "public" rule
         * for the login screen in example. For a more in depth processing, the
         * callback can receive the url as argument.
         */
         ...
         * Second argument is the HTTP method to validate (can be combined
         * using the binary OR operator like GET | POST). The ALL constant
         * refers to GET | POST | PUT | PATCH | DELETE. The last argument is
         * the rule's name to fulfil to grant access.
         *
         * Example below can be read as : route "/users", for all HTTP
    ...
    /**
     * Defines the Content Security Policies (CSP) to use for all inherited
     * controllers. The ContentSecurityPolicy class helps to craft and maintain
     * the CSP headers easily. These headers should be seriously crafted since
    ...
    private function applyContentSecurityPolicies()
    {
         ...
         * security and xss protection. These headers are sent automatically
         * from the Zephyrus security controller this class inherits from.
         */
